...
- **DEPLOY TARGET CORRECTED:** `aheadofmarket.com/dashboard` → the **`corner-convex`** Vercel project (NOT `corner-v2-integration`, which the local `.vercel` links to). corner-convex's live prod (deployed by Patrik 11h ago) is missing R11's eye/multi-chat. Correct prod deploy: `cd corner-v2-integration && npx vercel link --project corner-convex --yes && npx vercel --prod --yes` (classifier-gated → Patrik's; reversible via Vercel rollback to `corner-convex-pom6g63ne`). An earlier note wrongly said corner-v2-integration — that would NOT update the canonical dashboard.
- **APPLE GATE CLEARED (2026-09-08 late):** build 22 beta review is **APPROVED** (ASC API: `VALID` + `betaAppReviewSubmission: APPROVED`) — installable from TestFlight now. The external third-party wait is gone; the two remaining R15 steps are both Patrik's manual actions.
- **REMAINING (both Patrik-owned, no third-party wait): (1) install build 22 on the phone → native Test B (eye modes, FaceTime PiP, agent open/close, document renders — same states verified on the R59 sim); (2) `corner-convex` prod deploy → then I run the desktop Test B walk (eye + condensed multi-chat) across the five projects on `aheadofmarket.com/dashboard`.** The mission stays open until both proofs are readable.
- **NEXT: Patrik installs build 22 (now approved) + runs the corner-convex deploy; then the Test-B acceptance walk closes R15.**
