...
### R82 — Harness study, Phase 0: learn from the best harness code before building (2026-09-17)

Patrik, 2026-09-17: "we're trying to patch but we aren't creating the real solution for the chat that it needs" and "before doing this I want to study real code that does the same thing we're trying to do but from the best examples we can find structurally so we can learn instead of invent." Plan: `/root/.claude/plans/we-have-to-take-wiggly-crane.md` (session plan; the diagnosis and the three phases). Frame corrected by Patrik in the plan review: behind the chat is the LLM of choice; the harness is the agency's skills in the form of one hard-working, easy-to-talk-to agent; order of operations, not rules; "never send" is a bad rule, approvals are a tier in the order of operations the way Cursor does it; latency does not matter while real steps fire; build for three-years relevance; aom world owners are patrikmatheson@gmail.com and Ash's account.

Deliverable of this round: one structural brief per exemplar harness (loop, skills/tools, memory, order of operations + approvals, steps/events to the UI, model portability) under `corner/missions/corner-v2/research/harness-study/`, plus `PATTERN.md` (the shared pattern, what fits an agency owner on a phone, adopt vs adapt). These belong to `corner:assistant-gauntlet` (AOM-EA) per the plan; they are written here because this session pushes to aom-studio only. Move with `git mv` when convenient.
