...
- **4a. Knowing the user (more JSON?).** The ledger exists, but what *seals* actually knowing the user is likely more JSON — **where does it live, and how do we ingest it from other LLMs on day one with minimal friction** ("paste this and give me the input")?
  ...
  - **BUILT (2026-09-09):** `scripts/onboard_import.py` — the paste-this flow. `--prompt` prints the extraction prompt the user drops into their other LLM (dump everything as one strict JSON object). `normalize()` parses the pasted JSON (robust to ```json fences) into proposed ledger `fact` rows for review — it does NOT auto-write (house rule: ledger facts are proposals the user confirms). Verified via `--selftest`. So 4a's mechanism exists; open (Patrik): where the imported facts finally live (ledger `fact` deeds under an `onboarding` subject is the current default) and whether to add a one-tap importer in the app.
...
### CORRECTION (2026-09-09, Patrik questioned the "rate limit") — Arcade is fully open, connect flow verified
My earlier "rate-limited" claim was WRONG. The 403s were **Cloudflare error 1010** (bot-signature block on raw urllib requests missing a browser User-Agent) — `/v1/health` happened to pass, so I misread it. With a normal `User-Agent` header the API is fully accessible.

Verified live (2026-09-09):
- Catalog = **8,252 tools / 125 toolkits**. Email providers: **Gmail (30 tools), MicrosoftOutlookMail (30) + OutlookMail (13)**. Also GitHub, Slack, Notion, Google Calendar/Docs/Drive/Sheets/Slides, Dropbox, X, LinkedIn, Teams, Excel/Word, Mailchimp, etc.
- **Connect flow works:** `POST https://api.arcade.dev/v1/tools/authorize {tool_name, user_id}` (with a browser UA) returns a real Google OAuth URL (`accounts.google.com/.../auth`, scope `gmail.modify` + userinfo), an auth id `ar_...` to poll, `provider_id: arcade-google`, `status: pending`. Only the user's approval tap is untested (that's Patrik's OAuth).

BUILD PATH (Patrik green-lit "I'll connect if it's on TestFlight"): (1) an ADDITIVE corner-convex action `arcade.authorize(toolkit)`/`arcade.status(id)` holding `ARCADE_API_KEY` as a Convex env secret (the app can't hold the key); (2) the iOS Connections panel gets a "Connect" per provider → calls the action → opens the URL in Safari → polls status → flips the dot to connected; (3) ship TestFlight. Deploy note: the Convex action is a NEW module (additive, low-risk) but it's prod (brilliant-scorpion-163) + needs the env secret set — that deploy/env step is Patrik's call.
