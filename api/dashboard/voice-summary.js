...
// transcript he accumulated during the call gets POSTed here. We ask Gemini Flash
// to summarize it, then write ONE composed message to supabase-messages so the
...
// directly, Gemini is not a collaborator."
//
// Why this exists: terminal rooms (elon, studio) are live Claude Code sessions.
// Their text chat goes straight to the session via tmux relay (no Gemini). Voice
// is the one case where Gemini runs, because Gemini Live does the voice conversation.
// This endpoint closes the loop by feeding the voice output back to the terminal
// the same way typed chat does.
...
function buildPreamble({ agent, duration_secs, turn_count }) {
  const parts = []
  parts.push(`[VOICE CALL SUMMARY -- via Gemini, room=${agent}${duration_secs ? `, ${duration_secs}s` : ''}${turn_count ? `, ${turn_count} turns` : ''}]`)
  parts.push('')
  parts.push('Context: Patrik just had a voice conversation with Gemini to get this out faster.')
  parts.push('Treat it as Patrik speaking to you directly. Gemini is a voice-to-text shortcut,')
  parts.push('not a collaborator -- it is not awaiting a response from you and has no further role.')
...
async function callGeminiFlash(transcriptText) {
  ...
  if (!resp.ok) throw new Error(`Gemini ${resp.status}: ${data?.error?.message || 'unknown'}`)
...
export default async function handler(req, res) {
  ...
  let summary = ''
  try {
    summary = await callGeminiFlash(transcriptText)
  } catch (err) {
