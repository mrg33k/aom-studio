...
// Direct Google review link — opens the rate-and-review dialog for the
// "Wolfpack Companies LLC" listing (place ID pulled from the live Maps
// listing 2026-09-03; hex feature id 0x872b6da57fa25859:0x6ddf26b7ceffae34).
const GOOGLE_REVIEW_URL = 'https://search.google.com/local/writereview?placeid=ChIJWViif6VtK4cRNK7_zrcm320'

// Official multi-color Google "G" mark.
function googleG(size = 18) {
  return `<svg class="google-g" width="${size}" height="${size}" viewBox="0 0 48 48" aria-hidden="true">
...
export function renderHeader(page) {
  ...
  return `<header class="site-header" data-site-header>
    ...
    <div class="header-main wrap">
      ...
      <a class="header-review" href="${GOOGLE_REVIEW_URL}" target="_blank" rel="noopener">${googleG(20)}<span class="header-review-text"><strong>Review us on Google</strong><span>${reviewStars} 5.0 rated</span></span></a>
    ...
    <nav id="mobile-navigation" class="mobile-nav" aria-label="Mobile" hidden>
      ...
      <a class="mobile-review" href="${GOOGLE_REVIEW_URL}" target="_blank" rel="noopener">${googleG(18)}<span>Review us on Google</span>${reviewStars}</a>
...
<body>
  ${renderHeader(page)}
  <a class="review-strip" href="${GOOGLE_REVIEW_URL}" target="_blank" rel="noopener">${googleG(16)}<strong>Review us on Google</strong>${reviewStars}</a>
  <main id="main">
    ${renderBody(page)}
  </main>
  ${renderFooter()}
