// Corner Test Pass 20: R21 Acceptance -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p20-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p20-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p20-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p20-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p20-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p20-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p20-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 20 RESULTS (R21 Acceptance Test) ===\n')
  ...
  const outPath = `${DIR}/pass20-results.json`
