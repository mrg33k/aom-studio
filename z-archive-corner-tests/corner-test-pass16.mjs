// Corner Test Pass 16: R21 Smoke Test -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p16-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p16-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p16-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p16-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p16-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p16-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p16-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 16 RESULTS (R21 Smoke Test) ===\n')
  ...
  const outPath = `${DIR}/pass16-results.json`
