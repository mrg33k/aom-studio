// Corner Test Pass 23: R21 Acceptance -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p23-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p23-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p23-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p23-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p23-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p23-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p23-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 23 RESULTS (R21 Acceptance Test) ===\n')
  ...
  const outPath = `${DIR}/pass23-results.json`
