// Corner Test Pass 38: R21 Acceptance Test -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p38-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p38-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p38-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p38-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p38-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p38-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p38-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 38 RESULTS (R21 Acceptance Test) ===\n')
  ...
  const outPath = `${DIR}/pass38-results.json`
