// Corner Test Pass 29: R21 Acceptance -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p29-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p29-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p29-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p29-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p29-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p29-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p29-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 29 RESULTS (R21 Acceptance Test) ===\n')
  ...
  const outPath = `${DIR}/pass29-results.json`
