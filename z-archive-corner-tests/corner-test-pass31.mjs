// Corner Test Pass 31: R21 Acceptance -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p31-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p31-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p31-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p31-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p31-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p31-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p31-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 31 RESULTS (R21 Acceptance Test) ===\n')
