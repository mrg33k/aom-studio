// Corner Test Pass 37: R21 Acceptance Test -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p37-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p37-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p37-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p37-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p37-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p37-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p37-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 37 RESULTS (R21 Acceptance Test) ===\n')
  ...
  const outPath = `${DIR}/pass37-results.json`
