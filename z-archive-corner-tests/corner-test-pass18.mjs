// Corner Test Pass 18: R21 Acceptance -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p18-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p18-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p18-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p18-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p18-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p18-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p18-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 18 RESULTS (R21 Acceptance Test) ===\n')
  ...
  const outPath = `${DIR}/pass18-results.json`
