// Corner Test Pass 36: R21 Smoke Test -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p36-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p36-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p36-02-project-via-nav.png` })
      }
    }
    ...
    const typedText = `r21-smoke-test-${Date.now()}`
    ...
    await page.screenshot({ path: `${DIR}/p36-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p36-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p36-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p36-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 36 RESULTS (R21 Smoke Test) ===\n')
  ...
  const outPath = `${DIR}/pass36-results.json`
