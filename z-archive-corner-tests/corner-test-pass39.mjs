// Corner Test Pass 39: R21 Smoke Test -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p39-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p39-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p39-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p39-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p39-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p39-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p39-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 39 RESULTS (R21 Smoke Test) ===\n')
  ...
  const outPath = `${DIR}/pass39-results.json`
