// Corner Test Pass 28: R21 Acceptance -- Canonical Project-Chat URL + In-Chat Task Button
//
...
async function run() {
  ...
  try {
    ...
    await page.screenshot({ path: `${DIR}/p28-00-dashboard.png` })
    ...
    await page.screenshot({ path: `${DIR}/p28-01-project-chat.png` })
    ...
    if (!inputEl || !btnEl) {
      ...
      if (projectClicked) {
        ...
        await page.screenshot({ path: `${DIR}/p28-02-project-via-nav.png` })
      }
    }
    ...
    await page.screenshot({ path: `${DIR}/p28-03-input-filled.png` })
    ...
    await page.screenshot({ path: `${DIR}/p28-04-after-create.png` })
    ...
    await page.screenshot({ path: `${DIR}/p28-05-final.png` })

  } catch (err) {
    log('RUNTIME', 'FAIL', err.message.slice(0, 200))
    await page.screenshot({ path: `${DIR}/p28-error.png` }).catch(() => {})
  }

  await browser.close()

  // ── Summary ────────────────────────────────────────────────────────────────
  console.log('\n=== PASS 28 RESULTS (R21 Acceptance Test) ===\n')
  ...
  const outPath = `${DIR}/pass28-results.json`
