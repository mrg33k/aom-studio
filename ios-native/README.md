...
`Package.resolved` is regenerated by Xcode; there are no third-party packages in the app target
...
| What | How |
|---|---|
| Auth | Convex Auth (password provider) over plain HTTP, email+password, Keychain session with JWT refresh (ConvexAuth.swift) |
| The world/tenant | the signed-in user's `user_metadata.world` — **never** defaulted |
| History | Convex `messages:list` / `messages:getThread` (raw HTTP, ConvexService.swift) |
| Send | Convex `messages:send` with the Convex JWT; idempotent on clientMessageId |
...
| Live updates | Reconcile poll on `messages:list` (1.0 s during a turn, 2.5 s idle); no socket |
...
| Assign | `messages:send` into the agent's room — the row IS the assignment |
...
There is no API key in `Config.swift` any more. Identity is the Convex JWT from sign-in; every privileged call carries it and the server verifies it.
