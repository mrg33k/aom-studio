...
//
// THE FORM IS SPLIT FROM THE CHROME. `ReviewDecisionForm` is a pure view over its state,
// which is what lets the render gallery draw each verdict state on purpose — the whole
// sheet could not be snapshotted at all (ImageRenderer cannot draw a NavigationStack;
// it produced a black bar and then an unsupported-content glyph), so the states that
// matter most were the ones nobody could look at.

import SwiftUI

/// Which step of the decision the user is on.
enum ReviewDecisionMode: Equatable {
    case choosing
    case writingChanges
...
struct ReviewDecisionSheet: View {
    ...
    @State private var mode: ReviewDecisionMode = .choosing
        ...
        NavigationStack {
            ScrollView {
                ReviewDecisionForm(
                    ...
                    busy: busy,
                    failure: failure,
                    onMode: { mode = $0; failure = nil },
                    onSubmit: { action in Task { await submit(action) } }
                )
            }
    ...
    private func submit(_ action: ReviewAction) async {
        ...
        if ok {
            ...
            // The CAUSE only. The card this lands in is already titled "Nothing was
            // recorded", so the store's full self-contained sentence would print that
            // fact twice, one line under its own heading.
            failure = review.notice?.reason ?? "The server did not record the decision."
        }
    }
}
...
struct ReviewDecisionForm: View {
    let attachment: Attachment
    let locationLabel: String
    let mode: ReviewDecisionMode
    @Binding var notes: String
    var busy: Bool = false
    var failure: String?
    var onMode: (ReviewDecisionMode) -> Void = { _ in }
    var onSubmit: (ReviewAction) -> Void = { _ in }

    /// Content only — the scroll container belongs to whoever presents this. Keeping the
    /// ScrollView in here made the form unsnapshotable too (ImageRenderer draws a
    /// ScrollView's frame and none of its content), and a container is the presenter's
    /// decision anyway.
    var body: some View {
    ...
    private var choices: some View {
        VStack(spacing: Theme.s2) {
            Button { onMode(.confirmingApprove) } label: {
                decisionRow(
            ...
            Button { onMode(.writingChanges) } label: {
                decisionRow(
    ...
    private var changesForm: some View {
        VStack(alignment: .leading, spacing: Theme.s3) {
            // The label and its explanation are ONE object and sit tighter to each
            // other than either does to the field. At an equal 12pt all round, the
