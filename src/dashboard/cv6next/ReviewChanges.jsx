...
// request-changes decision): one bullet per comment, timecode first when the
// comment anchors to a video frame.
export function compileChanges(pins = []) {
  return (pins || [])
...
export function ReviewChangesOverlay({ pins = [], title = '', onSendBack = () => {}, onClose = () => {} }) {
  ...
  const list = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: isMobile ? '46vh' : '52vh', overflowY: 'auto' }}>
      {pins.map((p) => (
      ...
      {!pins.length && (
        <div style={{ padding: '18px 0', textAlign: 'center', color: 'var(--muted)', fontSize: 13 }}>No comments yet — pin one on the deliverable first.</div>
      )}
    </div>
  );
    ...
    <div style={{ marginBottom: 14, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: isMobile ? 16 : 14, fontWeight: 600, color: 'var(--fg)', marginBottom: 4 }}>
          Changes · {pins.length} {pins.length === 1 ? 'note' : 'notes'}
        </div>
  ...
  const footer = (
    <div style={{ display: 'flex', gap: 10, marginTop: 16, flexDirection: isMobile ? 'column' : 'row' }}>
      <button
        ...
        style={{ flex: 1, padding: isMobile ? '12px 14px' : '10px 14px', background: 'var(--accent)', border: 'none', borderRadius: 10, color: '#fff', fontSize: isMobile ? 14 : 13, fontWeight: 600, cursor: pins.length ? 'pointer' : 'not-allowed', opacity: pins.length ? 1 : 0.5, fontFamily: 'var(--font-sans)' }}
      >
        Send back to agent
      </button>
      <button
  ...
  if (!isMobile) {
    return (
      <div style={{ position: 'fixed', inset: 0, zIndex: 40 }} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '90%', maxWidth: 460, background: 'var(--ground)', border: '1px solid var(--hair)', borderRadius: 16, padding: 20, zIndex: 41, boxShadow: '0 24px 48px -12px rgba(0,0,0,0.4)' }}>
          {header}
          {list}
          {footer}
        </div>
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 40 }} onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, zIndex: 41, background: 'var(--ground)', borderRadius: '20px 20px 0 0', border: '1px solid var(--hair)', borderBottom: 'none', padding: '16px 16px calc(16px + env(safe-area-inset-bottom))', maxHeight: '75vh', overflowY: 'auto', boxShadow: '0 -12px 30px rgba(0,0,0,0.4)' }}>
        {header}
        {list}
        {footer}
      </div>
