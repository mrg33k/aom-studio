...
// R75-b6 -- Canonical recipe book component.
//
// Fetches from /api/dashboard/skills (scope-aware endpoint) so non-AOM tenants
// only see public skills. AOM tenants see the full catalog.
//
// Two surfaces:
//   mode='project'  Grand catalog view — every recipe by category, searchable.
//   mode='agent'    Filtered to the agent's skill domain (?agent=<slug>).
//
// Click a recipe → InputCollector modal → onFire({ recipe, input }) callback.
// The parent (ThreadView / ProjectChatView) fires the text into the chat.
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          ...
          {recipe.alias && (
            <span style={{ fontSize: 10.5, fontWeight: 500, color: C.dim }}>
              {recipe.alias}
            </span>
          )}
        </div>
...
export default function RecipeBook({ mode = 'project', agentSlug, tenantId, onClose, onFire }) {
  ...
  useEffect(() => {
    ...
    if (mode === 'agent' && agentSlug) params.set('agent', agentSlug)
    if (tenantId) params.set('tenant', tenantId)
    const qs = params.toString() ? `?${params}` : ''

    fetch(`/api/dashboard/skills${qs}`)
