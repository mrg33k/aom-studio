...
// Ported verbatim from the CV6 design system (components/navigation/SideRail.jsx).
 ...
 */
export function SideRail({ items = [], active, onSelect, onMenu, utilities = ['search', 'theme', 'alerts'], onUtility, style = {} }) {
  const Divider = () => (
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, marginTop: 14, flex: 'none' }}>
        {utilities.map((u) => (
          <div key={u} title={u} onClick={() => onUtility && onUtility(u)} style={{ position: 'relative', width: 38, height: 38, borderRadius: 19, background: 'var(--chip)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--muted)', cursor: 'pointer' }}>
            {util[u]}
