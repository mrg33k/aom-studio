...
/* shared kicker + browser frame for fresh heroes */
...
.r5 .edhead{font-size:118px;line-height:.84;margin:36px 0;}
.r5 .edrow{display:grid;grid-template-columns:.85fr 1.5fr;gap:44px;align-items:end;}
.r5 .edlede{font-size:19px;line-height:1.55;color:var(--ink-300);max-width:34ch;}
.r5 .edstrip{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;}
.r5 .edframe{position:relative;aspect-ratio:4/3;overflow:hidden;border:1px solid var(--ink-700);cursor:pointer;}
.r5 .edcap{position:absolute;left:0;right:0;bottom:0;z-index:2;padding:10px;font-size:10px;letter-spacing:.1em;text-transform:uppercase;color:var(--paper);font-weight:600;background:linear-gradient(transparent,rgba(0,0,0,.85));}
.r5 .edcredits{display:flex;gap:32px;flex-wrap:wrap;margin-top:42px;padding-top:22px;border-top:1px solid var(--ink-700);font-family:var(--display);font-weight:800;text-transform:uppercase;font-size:15px;color:var(--ink-300);}

/* ---- 3: Split — reel beside a real site ---- */
.r5 .herosplit{background:var(--paper);color:var(--ink);flex-direction:column;justify-content:center;gap:36px;}
.r5 .splhead{text-align:center;}
.r5 .splhead .kick{margin-bottom:18px;}
.r5 .splhead h1{font-size:60px;line-height:.94;max-width:20ch;margin:14px auto 0;}
.r5 .splhead .cta{display:flex;gap:12px;justify-content:center;margin-top:26px;flex-wrap:wrap;}
.r5 .splgrid{display:grid;grid-template-columns:1fr 1fr;gap:18px;}
...
function BentoHero({ variant, portrait, setVideo }) {
  ...
  const Intro = (
    <div className="bcell bintro">
      <span className="kick"><span className="ledot" /> Story · Film · Web</span>
    ...
    <section className="scene" ref={ref}>
      <div className="scenepin">
        ...
        <motion.div className="scenehead" style={{ y: headY, scale: headScale }}>
          <span className="kick"><span className="ledot" /> What we are</span>
          <h2 className="disp">Not a video studio.<br /><span className="gold">A story studio.</span></h2>
          <div className="scenestat"><b className="disp">100+</b> projects shipped for tech, construction, and nonprofit clients since 2020. Film, web, and the distribution strategy to make it matter.</div>
        </motion.div>
      </div>
    </section>
  );
...
const HPROOF = [
  [‘3 videos’, ‘produced for ISA Energy — running in investor meetings to help raise capital’],
  [‘1,000+’, ‘attended Space Congress where we drove traffic to SpaceOS, a platform we built’],
  [‘4 leads/mo’, ‘organically from Ambition Mechanical’s new site, before a single ad dollar’],
];
        ...
        <motion.div className="htrack" style={{ x }}>
          ...
          <div className="hpanel hp-intro">
            ...
            <div className="hp-inner">
              ...
              <p className="hp-lede">Most people around Phoenix know us as a video company. Others know us as a web shop. The truth is, we're neither — we're a storytelling company that just happens to make films and build websites.</p>
...
export default function HomeR5Preview() {
  ...
  return (
    <div className="r5" ref={rootRef}>
      ...
      {/* MANIFEST SCENE — pinned story statement with parallax shapes + film clips */}
      <ManifestScene />
      ...
      <section className="svc">
        ...
        <div className="svctop">
          ...
          <p className="svclede">Everything we do falls into two parts: <strong>Marketing</strong> — the materials the story stands on. A website, a brand film, a quiz a prospect takes. And <strong>Promotion</strong> — how it gets out into the world. Ads, influencer posts, email and text campaigns. A great billboard in your basement helps nobody. We help you make remarkable things and get them seen.</p>
        </div>
      ...
      <footer className="foot"><div className="wrap">
        ...
        <div className="base">
          <div className="sig">A storytelling studio<b>.</b></div>
