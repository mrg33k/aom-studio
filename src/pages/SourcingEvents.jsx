...
const EVENT_TYPE_COLORS = {
  conference: { bg: 'rgba(232,93,38,0.1)',   border: 'rgba(232,93,38,0.4)',  text: '#FDBA74' },
...
function DatePill({ dateStr }) {
  ...
  return (
    <div style={{
      width: 52, flexShrink: 0,
      background: isUpcoming ? `${V.orange}15` : V.card2,
      border: `1px solid ${isUpcoming ? `${V.orange}40` : V.border}`,
      borderRadius: 8, padding: '8px 0', textAlign: 'center',
    }}>
      <div style={{ fontSize: 9, fontWeight: 700, fontFamily: V.mono, color: isUpcoming ? V.orange : V.dim, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
        {month}
      </div>
...
// ─── Event Card ───────────────────────────────────────────────────────────────
function EventCard({ listing, company, onClick }) {
  ...
  return (
    ...
    >
      <DatePill dateStr={listing.event_date} />
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          <EventTypeBadge eventType={listing.event_type} />
          ...
          {listing.event_date && (
            <span style={{ fontSize: 11, color: V.dim, fontFamily: V.mono }}>
              {formatEventTime(listing.event_date)}
            </span>
          )}
          {isPast && (
            <span style={{ fontSize: 11, color: V.dim, fontFamily: V.mono, marginLeft: 'auto' }}>Past event</span>
          )}
        </div>
      </div>
    </div>
...
// ─── Calendar View ────────────────────────────────────────────────────────────
function CalendarView({ listings, onSelectListing }) {
  ...
  return (
    <div style={{ background: V.card, border: `1px solid ${V.border}`, borderRadius: 10, overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: `1px solid ${V.border}` }}>
        <button onClick={prevMonth} style={{ background: 'none', border: 'none', color: V.muted, cursor: 'pointer', padding: '4px 8px', fontSize: 16 }}>
          ‹
        </button>
        <div style={{ fontSize: 14, fontWeight: 700, fontFamily: V.syne, color: V.text }}>
          {viewDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
        </div>
        <button onClick={nextMonth} style={{ background: 'none', border: 'none', color: V.muted, cursor: 'pointer', padding: '4px 8px', fontSize: 16 }}>
          ›
        </button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', borderBottom: `1px solid ${V.border}` }}>
        {DOW.map(d => (
          <div key={d} style={{ padding: '8px 4px', textAlign: 'center', fontSize: 10, fontFamily: V.mono, color: V.dim, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            {d}
          </div>
        ))}
      </div>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: isToday ? 'rgba(232,93,38,0.05)' : 'transparent',
                }}
              >
                {day && (
                  <>
                    <div style={{
                      fontSize: 12, fontFamily: V.mono, fontWeight: isToday ? 800 : 500,
                      color: isToday ? V.orange : V.dim,
                      marginBottom: 4,
                    }}>
                      {day}
                    </div>
                    {events.map((ev, ei) => (
                      <div
                        key={ei}
                        onClick={() => onSelectListing(ev)}
                        style={{
                          background: V.orange, borderRadius: 3,
...
// ─── Event Detail Modal ───────────────────────────────────────────────────────
function EventModal({ listing, company, onClose }) {
  useEffect(() => {
    ...
    >
      ...
      >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: V.muted, fontFamily: V.mono }}>
                    {formatEventTime(listing.event_date)}
                    {listing.event_end_date && ` – ${formatEventTime(listing.event_end_date)}`}
                  </div>
                </div>
              </div>
            )}
            {(listing.event_location || listing.virtual_url) && (
              <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                ...
                <div>
                  ...
                  {listing.virtual_url && (
                    <a href={listing.virtual_url} target="_blank" rel="noreferrer"
                      style={{ fontSize: 12, color: V.blue, fontFamily: V.space, textDecoration: 'none' }}>
                      Join Online
                    </a>
                  )}
                </div>
            ...
            {listing.organizer && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 13, color: V.muted, fontFamily: V.space }}>
                  Organized by {listing.organizer}
                </span>
              </div>
            )}
          </div>
        ...
        {listing.contact_email && (
          <a
            href={`mailto:${listing.contact_email}?subject=RSVP: ${encodeURIComponent(listing.title)}`}
            style={{
          ...
          <div style={{ display: 'flex', background: V.card, border: `1px solid ${V.border}`, borderRadius: 8, overflow: 'hidden' }}>
            {[
              ...
              <button
                ...
                style={{
                  background: viewMode === mode.key ? 'rgba(255,255,255,0.08)' : 'transparent',
                  border: 'none', color: viewMode === mode.key ? V.text : V.dim,
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          {EVENT_TYPES.map(et => (
            <button key={et.key} onClick={() => setEventType(et.key)} style={{
              background: eventType === et.key ? 'rgba(255,255,255,0.08)' : 'transparent',
              border: `1px solid ${eventType === et.key ? V.borderHov : V.border}`,
              color: eventType === et.key ? V.text : V.muted,
              borderRadius: 6, padding: '6px 12px', fontSize: 12,
              fontWeight: 600, fontFamily: V.space, cursor: 'pointer', whiteSpace: 'nowrap',
              transition: 'all 0.15s',
            }}>
          ...
          <button onClick={() => setUpcomingOnly(u => !u)} style={{
            background: upcomingOnly ? 'rgba(232,93,38,0.12)' : 'transparent',
            border: `1px solid ${upcomingOnly ? 'rgba(232,93,38,0.5)' : V.border}`,
            color: upcomingOnly ? '#FDBA74' : V.muted,
            borderRadius: 6, padding: '6px 12px', fontSize: 12,
            fontWeight: 600, fontFamily: V.space, cursor: 'pointer', whiteSpace: 'nowrap',
            transition: 'all 0.15s',
          }}>
