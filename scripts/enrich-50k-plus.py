#!/usr/bin/env python3
"""
Enrich US municipalities (>= 50k pop) with government contact data via Apollo.io.

Fork of enrich-municipality-contacts.py with the population filter widened to
>= 50000 (no upper bound). Reuses the same .enrich-progress.json checkpoint
file so the 285 municipalities already processed in the 100k-500k band are
skipped automatically. Resumable: progress is saved after every item and the
full JSON dataset is rewritten every 10 items so a crash never loses more
than the last partial batch.
"""
...
def main():
    ...
    targets = [p for p in places if (p.get("population_2020") or 0) >= 50000]
    print(f"Target (>= 50k): {len(targets):,}")

    progress = load_progress()
    processed_set = set(progress["processed"])
    todo = [p for p in targets if p["geoid"] not in processed_set]
    ...
    for i, place in enumerate(todo, 1):
        ...
        # Crash-safe: rewrite the full JSON every 10 items so an interruption
        # never loses more than the last partial batch.
        if i % 10 == 0:
            save_data(data)
            print(f"    -- checkpoint: dataset written ({i}/{len(todo)})")

        time.sleep(0.5)

    # Final write — ensure the tail (last <10 items) is persisted.
    if "metadata" not in data:
        data["metadata"] = {}
    data["metadata"]["enrichment"] = {
        "last_run": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
        "target_population_band": ">=50k",
