...
test('chat composer has two rows, Plan mode metadata, options, and live progress text', async ({ page }) => {
  ...
  await expect(page.getByTestId('cv6-step-card')).toContainText(/Inspecting the active CV6 route|Reading your message/)
...
test('Review changes compiles checklist and anchored notes into one send-back', async ({ page }) => {
  ...
  const notes = page.getByPlaceholder('Type additional notes for the agent…')
  await expect(notes).toBeVisible()
  await notes.fill('Please tighten the opening and keep the existing visual direction.')
