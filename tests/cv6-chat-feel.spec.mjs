...
// real mobile ChatLifecycle + rich composer via the no-auth demo fixture;
// Playwright owns the POSTs, so the live send path is exercised without a workspace.
...
test('day folds open on tap without losing the thread', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await page.goto(`${BASE}/dashboard?cv6=1&demo=mobile-chat-lifecycle`, { waitUntil: 'domcontentloaded' })
  const head = page.locator('.goalcard .gc-head').first()
  await head.waitFor({ state: 'visible', timeout: 15_000 })
  await head.click()
  await expect(page.locator('[data-cv6-message-thread][data-mode="day-folded"]').getByText('Older mobile reply stays inside the folded day.')).toBeVisible()
  // Latest-day content is still present after the fold opened (no thread loss).
  await expect(page.locator('[data-cv6-message-thread][data-mode="latest-day"]').getByText('Mobile long message clamp proof begins here.')).toBeVisible()
})
